...
namespace MGF.Mappers
{
    public abstract class MapperBase<T> where T : DomainBase 
    {
        ...
        public T Save(T obj)
        {
            if (null == obj) // Prevents object null exceptions trying to compare obj using it's Equals operator.
            {
            ...
            if (obj.IsSavable)
            {
                // throw new InvalidOperationException("Operation cannot be completed on object current state.");
            }
        ...
        /// <summary>
        /// Map an entity over to an actual domain object. Provides separation from EF6 vs NHibernate classes.
        /// </summary>
        ...
        /// <summary>
        /// Updates the object to the actual database, until update is called, the entity might have unsaved changes,
        /// denoted by the IsDirty flag.
        /// </summary>
        /// <param name="domainObject"></param>
